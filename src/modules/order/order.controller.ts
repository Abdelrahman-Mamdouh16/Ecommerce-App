...
export const createOrder = async (
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (couponCode) {
        const couponResult = await getCouponForOrder(
          couponCode,
          subtotal,
          session,
        );
      ...
      for (const item of stockUpdates) {
        ...
        item.product.soledItems =
          (item.product.soledItems || 0) + item.quantity;
...
export const getOrderById = async (
  ...
  try {
    ...
    if (req.user.role !== "admin" && !req.user._id.equals(order.user)) {
      return next(
        new Error("Unauthorized to access this order", { cause: 403 }),
      );
    }
...
export const cancelOrder = async (
  ...
  try {
    ...
    if (req.user.role !== "admin" && !req.user._id.equals(order.user)) {
      return next(
        new Error("Unauthorized to change this order", { cause: 403 }),
      );
    }
    ...
    if (!allowedStatuses.includes(order.status)) {
      return next(
        new Error(
          "This order can no longer be cancelled in its current status",
          {
            cause: 400,
          },
        ),
